...
import java.util.ArrayList;// DEBEN AGREGAR ESTO
...
public class Tetris extends JFrame implements KeyListener {
    ...
    public Tetris(){
        ...
        for(int i=0;i<arr.size();i++){
            if(usuario.equals(arr.get(i).getnombrejugador()) && "Tetris".equals(arr.get(i).getnombrejuego())){// EN CADA JUEGO CAMBIAN EL NOMBRE DEL JUEGO DONDE DICE Tetris
        ...
        if(lugar==-1){
            Puntos pu=new Puntos("Tetris",usuario,0,0);// AQUI TAMBIEN CAMBIEN EL NOMBRE DEL JUEGO
            arr.add(pu);
...
//             HASTA AQUI, EN EL LUGAR DONDE UDS QUIERAN GUARDAR LOS PUNTOS
        if ((b[
                4+prof[pos[0]][0][rand-1].x][prof[pos[0]][0][rand-1].y].getBackground() == Color.DARK_GRAY) &&
